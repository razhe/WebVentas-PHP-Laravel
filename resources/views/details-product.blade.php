@extends('layout')
@section('CSS')
    <link rel="stylesheet" href="{{url('static/css/details-product.css?v='.time())}}">
                ...
                <div class="cotainer-content">
                    ...
                    <div class = "product-price">
                        @if ($product[0] -> discount != 0)
                            <p class = "last-price">Precio anterior: <span>{{Config::get('configuracion-global.currency'). number_format($product[0] -> price, 0,  '', '.')}}</span></p>
                            <p class = "new-price">Nuevo precio: <span>{{Config::get('configuracion-global.currency').number_format($product[0]->price - round((($product[0] -> discount * $product[0] -> price) / 100)), 0,  '', '.')}}</span></p>
                        @else
                            <p class = "new-price">Precio: <span>{{Config::get('configuracion-global.currency'). number_format($product[0] -> price, 0,  '', '.')}}</span></p>
                        @endif
                    </div>
